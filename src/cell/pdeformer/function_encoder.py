...
def _init_he_uniform(module: nn.Module) -> nn.Module:
    r"""Apply He/Kaiming uniform weights with zero bias."""
...
class DeepSetFuncEncoder(nn.Module):
    r"""
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out = torch.mean(out, dim=-2)  # [..., dim_hidden]
        out = self.post_fn(out)  # [..., dim_out]
        return out


class WeightedDeepSetFuncEncoder(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        probs = F.softmax(weight.to(torch.float32), dim=-2)
        probs = probs.to(self.compute_dtype)
        feature_probs = feature * probs
        # [..., num_points, dim_hidden] -> [..., dim_hidden]
        out = torch.sum(feature_probs, dim=-2)
        out = self.post_fn(out)  # [..., dim_out]
        return out


class PerBranchFuncEncoder(nn.Module):
    r"""Encoder for functions defined on one-dimensional domain."""
    ...
    def __init__(self,
        ...
        self.function_encoder_list = nn.ModuleList([func_enc_cls(
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out = torch.stack(out, dim=-2)  # [..., num_branches, dim_out]
        return out


class Patched1DFuncEncoder(nn.Module):
    r"""Encoder for functions defined on one-dimensional domain.
    ...
    Examples:
        ...
        >>> num_points = 128
        >>> x = torch.tensor(np.random.randn(2, num_points, dim_in), dtype=torch.float32)
        >>> encoder = PatchedFuncEncoder(dim_in, dim_out, dim_hidden, num_layers,
        >>>                              patch_len, compute_dtype=torch.float32)
        >>> out = encoder(x)
        >>> print(out.shape)
        (64, 256)
    """
...
class Patched2DFuncEncoder(nn.Module):
...
class PatchSet2DFuncEncoder(nn.Module):
    r"""Encoder for functions defined on two-dimensional uniform grids."""
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = x.reshape(batch_size, n_patch_axis**2, patch_len**2 * dim_in)
        x = self.mlp(x)  # [bsz, npa_x*npa_y, dim_out]
        x = torch.mean(x, dim=-2)  # [bsz, dim_out]
        return x


class Patched2DConvFuncEncoder(nn.Module):
    r"""
    ...
    Examples:
        ...
        >>> x = torch.tensor(np.random.randn(2, resolution, resolution, dim_in), dtype=torch.float32)
    ...
    def __init__(self,
        ...
        self.patch_layer = nn.Conv2d(dim_in, dim_hidden,
                                     self.patch_len, self.patch_len).to(dtype=compute_dtype)
...
class Conv2dFuncEncoderBase(nn.Module):
    ...
    @staticmethod
    @abstractmethod
    def get_net(in_channels: int, out_dim: int, compute_dtype) -> nn.Module:
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        for flag, gauss_filter in zip(self.gaussian_flags, self.pre_gaussians):
            gauss_filter = gauss_filter.to(device=x.device, dtype=dtype)
            f_values = F.conv2d(f_values, gauss_filter, padding="same")
        ...
        net_in = torch.cat(net_in, dim=1)  # [bsz, in_channels, H, W]
        x = self.net(net_in)
    ...
    def _gaussian_filter_2d(kernel_size: int, n_sigma: float = 1.5) -> Tensor:
        ...
        gauss_filter = torch.from_numpy(gauss_filter)  # NDArray -> Tensor
        return gauss_filter
...
class Conv2dFuncEncoder(Conv2dFuncEncoderBase):
    ...
    def get_net(in_channels: int, out_dim: int, compute_dtype) -> nn.Module:
        layers = []
        layers.append(_init_he_uniform(nn.Conv2d(
            in_channels, 64, kernel_size=3, stride=2, padding=1, bias=True)).to(dtype=compute_dtype))  # [bsz, 64, H/2, W/2]
        layers.append(nn.GELU())
        layers.append(_init_he_uniform(nn.Conv2d(
            64, 128, kernel_size=3, stride=2, padding=1, bias=True)).to(dtype=compute_dtype))  # [bsz, 128, H/4, W/4]
        layers.append(nn.GELU())
        layers.append(_init_he_uniform(nn.Conv2d(
            128, 256, kernel_size=3, stride=2, padding=1, bias=True)).to(dtype=compute_dtype))  # [bsz, 256, H/8, W/8]
        layers.append(nn.GELU())
        layers.append(_init_he_uniform(nn.Conv2d(
            256, 512, kernel_size=3, stride=2, padding=1, bias=True)).to(dtype=compute_dtype))  # [bsz, 512, H/16, W/16]
        layers.append(nn.GELU())
        layers.append(nn.AdaptiveAvgPool2d((1, 1)))  # [bsz, 512, 1, 1]
        layers.append(nn.Flatten())  # [bsz, 512]
        layers.append(_init_he_uniform(nn.Linear(
            512, 512, bias=True)).to(dtype=compute_dtype))  # [bsz, 512]
        layers.append(nn.GELU())
        layers.append(_init_he_uniform(nn.Linear(
            512, out_dim, bias=True)).to(dtype=compute_dtype))  # [bsz, out_dim]
...
class Conv2dFuncEncoderV2(Conv2dFuncEncoderBase):
    ...
    def get_net(in_channels: int, out_dim: int, compute_dtype) -> nn.Module:
        ...
        conv_kwargs = dict(kernel_size=3, stride=2, padding=1, bias=True)
        net = nn.Sequential(
            _init_he_uniform(nn.Conv2d(in_channels, 16, **conv_kwargs)).to(dtype=compute_dtype),
            get_activation_fn(),  # [bsz, 16, H/2, W/2]
            _init_he_uniform(nn.Conv2d(16, 32, **conv_kwargs)).to(dtype=compute_dtype),
            get_activation_fn(),  # [bsz, 32, H/4, W/4]
            _init_he_uniform(nn.Conv2d(32, 64, **conv_kwargs)).to(dtype=compute_dtype),
            get_activation_fn(),  # [bsz, 64, H/8, W/8]
            _init_he_uniform(nn.Conv2d(64, 128, **conv_kwargs)).to(dtype=compute_dtype),
            get_activation_fn(),  # [bsz, 128, H/16, W/16]
            _init_he_uniform(nn.Conv2d(128, 256, **conv_kwargs)).to(dtype=compute_dtype),
            get_activation_fn(),  # [bsz, 256, H/32, W/32]
            _init_he_uniform(nn.Conv2d(256, out_dim, **conv_kwargs)).to(dtype=compute_dtype),
        )  # Output shape: [bsz, out_dim, H/64, W/64].
        return net
...
class Conv2dFuncEncoderV3(Conv2dFuncEncoderBase):
    ...
    def get_net(in_channels: int, out_dim: int, compute_dtype) -> nn.Module:
        ...
        conv_kwargs = dict(kernel_size=4, stride=4, bias=True)
        net = nn.Sequential(
            _init_he_uniform(nn.Conv2d(in_channels, 32, **conv_kwargs)).to(dtype=compute_dtype),
            get_activation_fn(),  # [bsz, 32, H/4, W/4]
            _init_he_uniform(nn.Conv2d(32, 128, **conv_kwargs)).to(dtype=compute_dtype),
            get_activation_fn(),  # [bsz, 128, H/16, W/16]
            _init_he_uniform(nn.Conv2d(128, out_dim, **conv_kwargs)).to(dtype=compute_dtype),
        )  # Output shape: [bsz, out_dim, H/64, W/64].
        return net
