...
class GraphormerEncoder(nn.Module):
    ...
    Args:
        ...
        ffn_embed_dim (int): The dimension of FFN's embedding (the number of output neurons
            corresponding to the first nn.Linear layer of the FFN). Default: ``3072``.
    ...
    Examples:
        ...
        >>> node_type = torch.tensor(np.random.randint(0, num_node_type, size=(2, 10, 1)), dtype=torch.long)
        >>> node_input_feature = torch.tensor(np.random.randn(2, 10, embed_dim), dtype=torch.float32)
        >>> in_degree = torch.tensor(np.random.randint(0, num_in_degree, size=(2, 10)), dtype=torch.long)
        >>> out_degree = torch.tensor(np.random.randint(0, num_out_degree, size=(2, 10)), dtype=torch.long)
        >>> attn_bias = torch.tensor(np.random.randn(2, 10, 10), dtype=torch.float32)
        >>> spatial_pos = torch.tensor(np.random.randint(0, num_spatial, size=(2, 10, 10)), dtype=torch.long)
    ...
    def __init__(
        ...
        if encoder_normalize_before:
            self.emb_layer_norm = nn.LayerNorm(self.embed_dim, eps=1e-5)
        else:
    ...
    def forward(
        ...
        padding_mask = torch.eq(node_type_, 0)  # [n_graph, n_node]
        ...
        x = x.permute(1, 0, 2)  # [n_graph, n_node, embed_dim] -> [n_node, n_graph, embed_dim]
